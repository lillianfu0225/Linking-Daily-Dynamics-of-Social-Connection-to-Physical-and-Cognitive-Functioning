...
################################################################################
# Function 1 ---- 
# rp(p)       Reformat p-value for printing on chart
rp=function(p) if (p<.0001)  return("p<.0001")  else  return(paste0("p=", sub("^(-?)0.", "\\1.", sprintf("%.4f", p))))

# Function 2 ----- 
# eval2()   evaluate a string as a function
...
# Function 3 ---- 
# two-line regression with glm() so that it has heteroskedastic robust standard errors
...
data.example <- read.csv("data/M2-R1_Mplus-respons.csv")  

 # data.example$Chronic.Conditions <- data.example$B1ChronSum16
 # data.example$Psychological.Distress <- data.example$B1SNEGAF
  #data.example$x1 <- data.example$STR_REAC

 # data.example$LifeSat <- data.example$B1SSATIS2
 # data.example$Stress.React <- data.example$STR_REAC

#3) Run analyses
#3.1 Testing if x1 has u-shaped effect on y
a=twolines(life_satisfaction ~ social_respons, data = data.example) # no
a=twolines(neg_affect ~ social_respons, data = data.example)
# a=twolines(neg_affect_11_item ~ social_respons, data = data.example)
a=twolines(pos_affect ~ social_respons, data = data.example) # no 
# a=twolines(pos_affect_10_item ~ social_respons, data = data.example)
a=twolines(sr_health_reverse ~ social_respons, data = data.example)
a=twolines(chronic_condition_remake ~ social_respons, data = data.example)
a=twolines(func_limitation ~ social_respons, data = data.example)

dat <- data.example %>%
  dplyr::select(life_satisfaction, social_respons) %>%
  na.omit()

life_satisfaction <- dat$life_satisfaction
social_respons <- dat$social_respons

a <- reg2(life_satisfaction ~ social_respons,
          xc = 0)
...
#3.3 Testing if x1  has u-shaped effect on y, controlling for Age, Sex, Educ
# a=twolines(life_satisfaction ~ social_respons + age + gender + education + pos_int_m + m2, 
#            data = data.example) # no
# a=twolines(neg_affect ~ social_respons + age + gender + education + pos_int_m + m2, 
#            data = data.example)
# a=twolines(pos_affect ~ social_respons + age + gender + education + pos_int_m + m2
#            + connection_m, 
#            data = data.example)
a=twolines(sr_health_reverse ~ social_respons + age + gender + education + pos_int_m + m2, 
           data = data.example)
a=twolines(chronic_condition_remake ~ social_respons + age + gender + education + pos_int_m + m2, 
           data = data.example)
a=twolines(func_limitation ~ social_respons + age + gender + education + pos_int_m + m2, 
           data = data.example)
